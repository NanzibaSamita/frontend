...
export default function SupervisorPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // fetch supervisor assignment eligibility
  ...
  const handleSubmit = async () => {
    if (!eligible) {
      setMessage("You are not eligible to request a supervisor (need ≥ 9 credits).");
      return;
    }
      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        {!eligible && (
          <div className="mb-6 p-3 bg-red-50 border border-red-300 text-red-800 rounded-lg">
            You are <strong>not eligible</strong> to request a supervisor (must have ≥ 9 credits).
          </div>
        )}
